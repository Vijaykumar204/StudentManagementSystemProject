...
public interface DailyAttendanceRepository extends JpaRepository<DailyAttendanceModel,Long> {
	...
	DailyAttendanceModel findStudentIdAndAttendanceDate(@Param("studentId") Long studentId, @Param("attendanceDate") LocalDate attendanceDate);


//	@Query("SELECT d FROM DailyAttendanceModel d WHERE  d.attendanceDate = :attendanceDate")
//	DailyAttendanceModel findStudentIdAndAttendanceDate( @Param("attendanceDate") LocalDate attendanceDate);


	 @Query("SELECT d FROM DailyAttendanceModel d " +
	           "WHERE d.attendanceDate = :attendanceDate " +
	           "AND d.attendanceStatus.code = :statusCode")
	    List<DailyAttendanceModel> findAbsentStudentIds(
